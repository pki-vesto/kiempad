...
function renderCurrentState(root: HTMLElement, state: RuntimeState): void {
  ...
  if (
    ...
    focusCentralSessionRenewalRecoveryStatus(root);
  }
  scheduleLocalNotifications(
    state.herinneringen,
    state.settings,
...
function focusCentralSessionRenewalRecoveryStatus(root: HTMLElement): void {
  const statuses = [
